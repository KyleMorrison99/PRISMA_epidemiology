...
# add diversity info
diversity_ratios_m <- data.frame(variable = c("gender_diversity", "country_diversity"),
...
png(filename = "circular barplot EDI.png", width = 6, height = 6, units = "in", type = "windows", res = 400)
p1
dev.off()
```
...
# map
p <- ggplot(map_data_final, aes(x = long, y = lat, group = group, fill = Count)) +
  geom_polygon(color = "white", size = 0.2) + 
  scale_fill_gradient(low = "lightblue", high = "darkblue",  na.value = "red") + # #F4A460
...
png(filename = "fig map.png", width = 5.5, height = 3.5, units = "in", type = "windows", res = 400)
p
dev.off()
```

...
```{r}
###############################################################################
# 1. DEFINE A 9-COLOR PALETTE AND A CUSTOM THEME
###############################################################################
my_colors_9 <- c(
  "#6C5B7B",
  "#C06C84",
  ...
  theme_minimal(base_size = 14) %+replace%
    theme(
      ...
      plot.title = element_text(
        face   = "bold",
        size   = 16,
        hjust  = 0,             # left-align the title
        margin = margin(b = 10) # space below title (pushes legend down)
      ),
...
###############################################################################
# 2. ORIGINAL PRISMA (2009 vs. 2020)
###############################################################################
PRISMA_original <- citations %>%
  filter(
    category  == "original",
    duplicate == "no",
    year      != 2025
...
original_count <- PRISMA_original %>%
  mutate(
    prisma_version = if_else(
      grepl("^PRISMA_2009", study_id),
      "PRISMA_2009",
      "PRISMA_2020"
    )
  ) %>%
  ...
  guides(fill = guide_legend(nrow = 2, byrow = TRUE)) +  # 2-row legend layout
  scale_x_continuous(breaks = 2009:2024, limits = c(2009, 2024), expand = c(0, 0)) +
  scale_y_continuous(
    limits = c(0, 35000),
  ...
  labs(
    title   = "Yearly Citations of the Original PRISMA Guidelines",
    x       = "Publication Year",
...
###############################################################################
# 3. TOPICAL EXTENSIONS
###############################################################################
citation_time_topic_extension <- citations %>%
  filter(
    category  == "topical",
    duplicate == "no",
    year      != 2025
...
earliest_topic_year <- citation_time_topic_extension %>% 
  left_join(
    bibliometric_information %>% select(study_id, PRISMA_id),
    by = "study_id"
  ) %>%
  filter(year != 2025, citations > 0) %>%
  group_by(PRISMA_id) %>%
  summarise(earliest_year = min(year, na.rm = TRUE), .groups = "drop") %>%
  arrange(earliest_year)

ordered_topic_ids <- earliest_topic_year$PRISMA_id

topic_extensions <- citation_time_topic_extension %>%
  left_join(
    bibliometric_information %>% select(study_id, PRISMA_id), 
    by = "study_id"
  ) %>%
  mutate(PRISMA_id = factor(PRISMA_id, levels = ordered_topic_ids)) %>%
  group_by(year, PRISMA_id) %>%
  summarise(total_citations = sum(citations, na.rm = TRUE), .groups = "drop") %>%
  ggplot(aes(x = as.numeric(year), y = total_citations, fill = PRISMA_id)) +
  geom_area(position = "stack", alpha = 0.8) +
  scale_fill_manual(values = my_colors_9) +
  ...
  labs(
    title   = "Yearly Citations of PRISMA Topical Extensions",
    x       = "Publication Year",
    y       = "Number of Citations",
    fill    = NULL,  # removes legend title
    caption = "Data: PRISMA Topical Extensions (2009–2024)"
  ) +
  my_theme()
...
###############################################################################
citation_time_method_extension <- citations %>%
  filter(
...
earliest_method_year <- citation_time_method_extension %>%
  left_join(
    bibliometric_information %>% select(study_id, PRISMA_id),
    by = "study_id"
  ) %>%
...
ordered_method_ids <- earliest_method_year$PRISMA_id

method_extensions <- citation_time_method_extension %>%
  left_join(
    bibliometric_information %>% select(study_id, PRISMA_id),
    by = "study_id"
  ) %>%
  mutate(PRISMA_id = factor(PRISMA_id, levels = ordered_method_ids)) %>%
  group_by(year, PRISMA_id) %>%
  summarise(total_citations = sum(citations, na.rm = TRUE), .groups = "drop") %>%
  ggplot(aes(x = as.numeric(year), y = total_citations, fill = PRISMA_id)) +
  geom_area(position = "stack", alpha = 0.8) +
  ...
  labs(
    title   = "Yearly Citations of PRISMA Methodological Extensions",
    x       = "Publication Year",
    y       = "Number of Citations",
    fill    = NULL,  # removes legend title
    caption = "Data: PRISMA Methodological Extensions (2009–2024)"
  ) +
  my_theme()

###############################################################################
# 5. DISPLAY & COMBINE PLOTS
###############################################################################
...
# Combine them (using patchwork for example)
 fig_combined <- original_count / topic_extensions / method_extensions
 fig_combined

# Save the combined figure if desired

...
# --- Helper: standardise nationality (optional but recommended) -------------
std_country <- function(x) {
  x <- stringr::str_squish(x)
  dplyr::na_if(x, "")
}

# --- Prep data: prefer web; fallback to genderize only if web NA -----------
prep_edi_author <- function(df) {
  df %>%
    mutate(
      ...
      web_clean = if_else(web_clean %in% c("not found","notfound","not-found"),
                          NA_character_, web_clean),
      ...
      gender_fb_std_hiconf = case_when(
        used_fallback &
          !is.na(gender_diversity_confidence) &
          gender_diversity_confidence > 95 ~ gender_fb_std,
...
# --- Global South country list (with China fixed) ---------------------------
...
# --- Ratios, including nationality-based global south -----------------------
compute_diversity_ratios <- function(data, global_south_countries) {
  data %>%
    mutate(
      nationality_clean        = std_country(nationality),
      nationality_global_south = nationality_clean %in% global_south_countries
    ) %>%
    group_by(PRISMA_id) %>%
    summarise(
      ...
      # Existing: proportion of author-rows whose geographic_diversity
      # string includes at least one global South country
      ...
      # New: proportion of authors whose nationality is global South
      global_south_nationality_ratio =
        mean(nationality_global_south, na.rm = TRUE),
...
diversity_ratios <- compute_diversity_ratios(
  edi_author_prepped,
  global_south_countries
)

# Overall averages
overall_avg_female              <- mean(diversity_ratios$female_ratio, na.rm = TRUE)
overall_avg_global_south_geo    <- mean(diversity_ratios$global_south_ratio, na.rm = TRUE)
overall_avg_global_south_nat    <- mean(diversity_ratios$global_south_nationality_ratio, na.rm = TRUE)

# Percentages (0–100 scale)
overall_percent_female           <- overall_avg_female * 100
overall_percent_global_south_geo <- overall_avg_global_south_geo * 100
overall_percent_global_south_nat <- overall_avg_global_south_nat * 100
...
```{r}
# =========================================
# 0. ECR flag in edi_author (ECR = NOT tenured)
# =========================================
edi_author <- edi_author %>%
  mutate(
    author_status_clean = str_to_lower(str_squish(author_status)),
    position_clean      = str_to_lower(str_squish(career_stage_publication)),
    position_clean      = na_if(position_clean, "cannot find"),
    
    # tenure: "yes" = professor / research fellow, otherwise "no"
    tenure = case_when(
      !author_status_clean %in% c("first", "last") ~ NA_character_,
      is.na(position_clean)                        ~ NA_character_,
      str_detect(position_clean, "profes")         ~ "yes",
      str_detect(position_clean, "research fellow")~ "yes",
      TRUE                                         ~ "no"
    ),
    
    # ECR flag: complement of tenure for first/last authors
    ecr = case_when(
      is.na(tenure)   ~ NA_character_,
      tenure == "no"  ~ "yes",  # ECR
      tenure == "yes" ~ "no"    # not ECR
    )
  )

# Get BOTH total count AND % ECR for FIRST authors
first_ecr_summary <- edi_author %>%
  filter(author_status_clean == "first", !is.na(ecr)) %>%
  summarise(
    total_first = n(),
    n_ecr       = sum(ecr == "yes", na.rm = TRUE),
    pct_ecr     = round(100 * n_ecr / total_first, 1)
  )



# =========================================
# 1. Create lingual_diversity2 in edi
#     "yes" if ≥1 language, "no" if 0, NA stays NA
# =========================================
edi <- edi %>%
  mutate(
    lingual_diversity2 = case_when(
      is.na(lingual_diversity)   ~ NA_character_,
      lingual_diversity > 1     ~ "yes",
      TRUE                       ~ "no"   # this implicitly means 0
    )
  )

# =========================================
# 2. Base “yes” percentages for checklist-level items
# =========================================
df1 <- dplyr::select(
  edi,
  PRISMA_id,
...
# =========================================
# 3. Add diversity ratios (assumes diversity_ratios already created)
# =========================================
diversity_ratios_m <- tibble(
...
# =========================================
# 4. Combine all metrics (incl. ECR)
# =========================================
df_fig <- dplyr::bind_rows(df_fig, diversity_ratios_m, ecr_df)

# =========================================
# 5. Clean and relabel variables
# =========================================
df_fig <- df_fig %>%
  mutate(
    variable = dplyr::recode(
      variable,
      "edi_considered"        = "EDI Considered",
      "lingual_diversity2"    = "Translated beyond English",
      "stakeholder_engagement"= "Stakeholder Engagement",
      "open_access"           = "Open Access Status",
      "author_diversity"      = "Woman Representation",
      "country_diversity"     = "Global South Affiliations",
      "first_ECR"             = "First authors ECR",
      "last_ECR"              = "Senior authors ECR"
    )
  )

# =========================================
# 6. Create circular barplot
# =========================================
figure_3 <- ggplot(
  df_fig,
  aes(
    x    = stringr::str_wrap(variable, 10),
    y    = percent_yes,
    fill = percent_yes
  )
) +
  ...
  geom_segment(
    aes(
      x    = stringr::str_wrap(variable, 10),
      y    = 0,
      xend = stringr::str_wrap(variable, 10),
      yend = 100
    ),
    linetype = "dashed",
    color    = "gray12"
  ...
  annotate(
    x = length(unique(df_fig$variable)) + 0.5,
    y = 25,
    label = "25%",
    geom  = "text",
    color = "gray12",
...
p_map <- ggplot(geo_data_complete,
  ...
  scale_fill_gradientn(
    colours = c("grey85", "grey40", "grey5"),  # light (low) → dark (high)
    name    = "Number of\nauthors",
    limits  = c(0, max(geo_data_complete$Count, na.rm = TRUE)),
    na.value = "grey92",
    guide   = guide_colorbar(
      barheight     = unit(0.8, "cm"),   # thin horizontal
      barwidth      = unit(10, "cm"),    # wide horizontal
      nbin          = 100,
      raster        = TRUE,
      frame.colour  = "black",
      frame.linewidth = 0.3,
  ...
  theme(
    legend.position  = "bottom",
    legend.key.width = unit(12, "cm"),   # extra wide
    legend.key.height = unit(1, "cm"),
    axis.text        = element_blank(),
    axis.ticks       = element_blank(),
    panel.grid       = element_blank(),
    plot.title       = element_text(size = 16, face = "bold"),
    plot.margin      = margin(t = 10, r = 10, b = 20, l = 10)  # more bottom space
  )
